...
The project reads configuration from environment variables or a TOML file parsed into `Settings`:
...
Configuration files can be supplied via `ccc --config path/to/settings.toml` and must follow the same
structure as the `Settings` model defined in `canvas_code_correction.config`.
