...
Grader tests are scripts that evaluate student submissions. They run inside the
grader container and produce results.

## Execution Environment

Each grader test runs in complete isolation, in a fresh Docker container created
just for a single student submission. This ensures:
...
Replace the `submission_files` logic with your own checks: run unit tests, style
checks, compile code, etc. Ensure your script writes the three output files.

## Packaging Your Work Package

Your work package should include:

1. **Grader test scripts** (required): Place your grader scripts in a directory
   (e.g., `grader/`)
2. **Dockerfile** (optional): Only needed if the default
   `jakob1379/canvas-grader:latest` image doesn't contain your required
   dependencies
...
Give these assets to your **CCC platform operator** for deployment. They will
configure your course, set up scheduling, and monitor results.
