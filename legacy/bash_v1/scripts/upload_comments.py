...
def upload_comments(sub, assignment):
    ...
    if submission.submission_comments:
        try:
            ...
            download_url(latest_comment_url, os.path.join("tmp", fname))
    ...
    if file_to_upload:
        ...
        out_str = bcolors.WARNING + "zip to upload not found in: ".upper() + sub + bcolors.ENDC
        print(out_str)
        return
...
def main():
    ...
    if args.parallel:
        if args.verbose:
            ...
            p_map(upload_comments, reports, assignment_for_reports, num_cpus=args.num_cpus)
    else:
