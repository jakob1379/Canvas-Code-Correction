...
def run(foldername):
    ...
    for result in result_files:
        if test_files_compiled[count] and not reachedTimeLimit[count]:
            with open(result) as f:
                content = f.readlines()
                (test_files_memory_freed[count], test_files_segfault[count]) = memoryFreed_segfaulted(
                    content)
        count += 1
    ...
    for result in result_files:
        ...
        if reachedTimeLimit[currentTestNumber]:
            outputFile += "-Time limit of " + \
                str(TIME_LIMIT) + \
                " seconds was reached - check for very bad code or infite loops.\n"
            ...
            # open result file
                ...
                with open(result) as f:
                    ...
                    while current < len(content):
                        (passed, description) = testPassed(
                            content[current+1], content[current])
                    ...
                    # check if all tests passed
                        ...
                        if test_files_memory_freed[currentTestNumber]:
                            ...
                            points_earned += total_points[test_files[currentTestNumber]]*(
                                1.0-minus_point_for_memory_leak)
                            points[currentTestNumber] = total_points[test_files[currentTestNumber]
                                                                     ]*(1-minus_point_for_memory_leak)
                    else:
                        if test_files_memory_freed[currentTestNumber]:
                            # " You missed " + str(total_points[test_files[currentTestNumber]]) + " points\n"
...
def memoryFreed_segfaulted(lines):
    ...
    for line in lines:
        if checkStrMem in line or "in use at exit: 16 bytes in 2 blocks" in line:
            memoryFree = True
