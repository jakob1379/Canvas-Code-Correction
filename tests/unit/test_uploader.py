...
def test_check_attachment_duplicate_exception_verbose() -> None:
    ...
    attachment = cast(
        "AttachmentWithUrl",
        {"url": "https://example.com/file.zip", "display_name": "file.zip"},
    )
...
def test_download_attachment_writes_response_content(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        uploader._download_attachment("https://example.com/file.zip", destination)

    assert destination.read_bytes() == b"helloworld"
    mock_get.assert_called_once_with("https://example.com/file.zip", stream=True, timeout=30)
