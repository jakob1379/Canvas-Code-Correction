...
parser.add_argument("-s", "--show", help="Show resulting plot", action="store_true")
parser.add_argument(
    "-n",
    "--num-cores",
...
parser.add_argument("--load-data", help="Load previous data if any available", action="store_true")
...
def load_data(course):
    ...
    scores = []
    pbar = Pbar.ProgressBar(redirect_stdout=True, max_value=Pbar.UnknownLength)
    counter = 0
    ...
    df = df.apply(pd.to_numeric, errors="coerce").fillna(df).reset_index(drop=True)
    df["passed"] = (
...
def plot_scores(df_in):
    ...
    num_students_passed = (
        df.groupby("user_id")
        .filter(lambda group: len(set(group.entered_grade)) == 1)
        .user_id.nunique()
    )
    students_passed = num_students_passed / nunique_students
    num_students_no_handins = (
        df.groupby("user_id")
        .filter(lambda group: all(group.entered_grade == "Not handed in"))
        .user_id.nunique()
    )
    students_no_handins = num_students_no_handins / nunique_students
    students_attempted_and_passed = num_students_passed / (
        nunique_students - num_students_no_handins
    )

    if args.verbose:
        print("Plotting...")
    fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, sharex=False, sharey=True, figsize=(12, 9))
    ...
    attempt_passed_line = Line2D(
        [0],
        [0],
        label=f"Attempted and passed: {100 * students_attempted_and_passed:.2f}% - {num_students_passed}/{nunique_students - num_students_no_handins:d}",
        color="tab:blue",
        linestyle="dashed",
    )
    ...
    ax1.legend(
        handles=list(line_handles) + list(bar_handles),
        title="Submissions",
        loc=4,
        framealpha=0.3,
        prop={"size": 8},
