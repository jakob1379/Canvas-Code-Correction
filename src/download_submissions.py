...
parser.add_argument("-v", "--verbose", help="sets verbosity", action="store_true")
parser.add_argument("-p", "--parallel", help="download in parallel", action="store_true")
parser.add_argument(
    "-n",
    "--num-cores",
    help="description",
    ...
    try:
        ...
        if url:
            ...
            folders_to_remove = [
                ...
                if str(sub.user_id) in old and os.path.join(file_name, "") not in old
            ]
            ...
            if os.path.join(directory, file_name, "") not in old_files:
                attachments_with_url = [att for att in sub.attachments if att.get("url")]
                for att in attachments_with_url:
                    final_path = os.path.join(directory, file_name, att.get("filename"))
                    if args.verbose:
...
def find_submissions():
    ...
    assignments = [a for a in course.get_assignments() if a.name in args.assignment]
    for assignment in assignments:
        ...
        if args.download == "all":
            ...
            submissions += [
                sub for sub in assignment.get_submissions() if sub.grade in ["incomplete", 0, "0%"]
            ]
            ...
            for sub in assignment.get_submissions(include="submission_comments"):
                ...
                comment_files = extract_comment_filenames(sub.submission_comments)
...
def main():
    ...
    if submissions:
        if args.parallel:
            ...
            p_umap(partial(download_submission), submissions, num_cpus=args.num_cores)
        else:
