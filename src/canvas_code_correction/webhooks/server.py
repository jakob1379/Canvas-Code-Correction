...
def get_rate_limited_settings(
    ...
    """Load settings and apply the per-course rate limit before verification."""
    if settings.webhook.enabled:
        rate_limit_check(course_block, settings.webhook.rate_limit, limiter)
    return settings
...
async def get_validated_webhook_request(
    request: Request,
    course_block: str,
    settings: Annotated[Settings, Depends(get_rate_limited_settings)],
